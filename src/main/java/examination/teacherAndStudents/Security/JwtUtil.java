...
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule;
import examination.teacherAndStudents.dto.SchoolAuthDto;
import io.jsonwebtoken.Claims;
import io.jsonwebtoken.Jwts;
import io.jsonwebtoken.SignatureAlgorithm;
import io.jsonwebtoken.io.Decoders;
...
public class JwtUtil {
    ...
    public String generateToken(String email, SchoolAuthDto schoolAuthDto) throws JsonProcessingException {
        Map<String, Object> extraClaims = new HashMap<>();
        if (schoolAuthDto != null) {
            String serializedSchool = objectMapper.writeValueAsString(schoolAuthDto);
            extraClaims.put("school", serializedSchool);
        } else {
            extraClaims.put("school", null);
        }
